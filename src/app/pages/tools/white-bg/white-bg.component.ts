...
@Component({
  selector: 'app-white-bg',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './white-bg.component.html',
  styleUrls: ['../tools-shared.css']
})
export class WhiteBgComponent {
  ...
  async process() {
    ...
    this.error.set('');
    try {
      const res = await this.processor.whiteBg(this.selectedFile()!);
  ...
  download() {
    ...
    if (!url) return;
    this.processor.downloadImage(url, 'white-bg.png');
  }
