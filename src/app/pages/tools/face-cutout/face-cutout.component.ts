...
@Component({
  selector: 'app-face-cutout',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './face-cutout.component.html',
  styleUrls: ['../tools-shared.css']
})
export class FaceCutoutComponent {
  ...
  async process() {
    ...
    this.error.set('');
    try {
      const res = await this.processor.faceCutout(this.selectedFile()!);
  ...
  download() {
    ...
    if (!url) return;
    this.processor.downloadImage(url, 'face-cutout.png');
  }
